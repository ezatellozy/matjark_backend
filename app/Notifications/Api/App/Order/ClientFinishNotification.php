...
class ClientFinishNotification extends Notification  implements ShouldBroadcast
{
    use Queueable;
    public $order;
    protected $via;
    ...
    public function __construct($order, $via)
    {
        ...
        $this->via = $via;
        $this->title = [
            'ar' => trans('app.orders.notifications.title.finish_order', ['order_id' => $this->order->id], 'ar'),
            'en' => trans('app.orders.notifications.title.finish_order', ['order_id' => $this->order->id], 'en'),
        ];
        $this->body = [
            'ar' => trans('app.orders.notifications.body.finish_order', ['order_id' => $this->order->id, 'client_name' => auth('api')->user()->fullname], 'ar'),
            'en' => trans('app.orders.notifications.body.finish_order', ['order_id' => $this->order->id, 'client_name' => auth('api')->user()->fullname], 'en'),
        ];
    }
