...
class BuyToGetOfferNewResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if ($this->buy_apply_on == 'special_categories')
        {
            ...
            $buy_apply = Category::whereIn('id', $this->buy_apply_ids)->where('position', 'second_sub')->get();

            $buy_apply->each(function ($category) {
        ...
        elseif ($this->buy_apply_on == 'special_products')
        {
            ...
            $buy_apply = Product::whereHas('productDetails', function($query)  {
                $query->whereIn('id', $this->buy_apply_ids);
            })->get();

            $buy_apply->each(function ($product)  {
                data_set($product, 'product_detail_ids', array_intersect($this->buy_apply_ids, $product->productDetails->pluck('id')->toArray()));
            });
        }
        ...
        if ($this->get_apply_on == 'special_categories')
        {
            ...
            $get_apply = Category::whereIn('id', $this->get_apply_ids)->where('position', 'second_sub')->get();

            $get_apply->each(function ($category) {
                data_set($category, 'root', root($category));
            });
        }
        elseif ($this->get_apply_on == 'special_products')
        {
            ...
            $get_apply = Product::whereHas('productDetails', function($query)  {
                $query->whereIn('id', $this->get_apply_ids);
            })->get();

            $get_apply->each(function ($product)  {
                data_set($product, 'product_detail_ids', array_intersect($this->get_apply_ids, $product->productDetails->pluck('id')->toArray()));
            });
        }
        ...
        return [
            ...
            "buy_apply"       => $this->buy_apply_on == 'special_categories' ? CategorySimpleNewResource::collection($buy_apply) : SimpleProductDetail2Resource::collection($buy_apply),
            ...
            "get_apply"       => $this->buy_apply_on == 'special_categories' ? CategorySimpleNewResource::collection($get_apply) : SimpleProductDetail3Resource::collection($get_apply),
