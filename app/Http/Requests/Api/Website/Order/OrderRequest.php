...
class OrderRequest extends ApiMasterRequest
{
    ...
    public function rules()
    {
        ...
        $data = [
            ...
            'redirect_url' => request()->pay_type == 'card' ? 'required|url' : 'nullable',
        ...
        if ($this->productHasValues($cart->cartProducts) == false) {
            throw new HttpResponseException(response()->json([
                'status' => 'fail',
                'message' =>  $lang != null ? 'some products are not available in your cart please check you cart again' : 'عفوا بعض المنتجات غير متاحة برجاء التحقق من المنتجات في السلة',
                'data' => null,
            ], 422));
        }
        ...
        if ($offerMessage != null && $offerMessage != true) {
            throw new HttpResponseException(response()->json([
                'status' => 'fail',
        ...
        $couponRow = ($this->code != null && Coupon::where('code', $this->code)->first() != null) ? Coupon::where('code', $this->code)->first() : null;

        $priceData =   $this->calculationItemsPrice(auth()->guard('api')->user()->cart, $couponRow);

        // info('balance');
        // info(auth()->guard('api')->user()->wallet->balance);
        // info($priceData['total']);
        // info('balance');
