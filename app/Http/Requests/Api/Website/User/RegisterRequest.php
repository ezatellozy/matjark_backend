...
class RegisterRequest extends ApiMasterRequest
{
    ...
    public function rules()
    {
        ...
        $validate_arr = [
            ...
            'email'    => 'nullable|email|unique:users,email,NULL,id,deleted_at,NULL|regex:/(.+)@(.+)\.(.+)/i', 
        ...
        if($country != null) {
            $validate_arr['phone'] = 'required|numeric|unique:users,phone,NULL,id,phone_code,' .  $this->phone_code.'|digits:'.$country->phone_number_limit;
        } else {
            $validate_arr['phone'] = 'required|numeric|unique:users,phone,NULL,id,phone_code,' .  $this->phone_code;
        }
