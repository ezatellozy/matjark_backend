...
class CouponRequest extends ApiMasterRequest
{
    ...
    public function rules()
    {
        ...
        $rules = [
            ...
            'discount_amount'   => $status . '|numeric|min:1',
    ...
    public function messages()
    {
        ...
        if($lang == 'en') {
            ...
            $arr['code.string'] = ' coupon is must be a string';
            $arr['code.required'] = ' coupon is required';
            $arr['code.unique'] = ' coupon must be unique';

            if(! empty(request()->apply_ids)) {
                foreach(request()->apply_ids as $key => $key2) {
                    
                    $arr['apply_ids.'.$key.'.exists'] = ' apply id '  .($key + 1). ' is not found';
                }
            }
            ...
            $arr['code.string'] = ' كود الخصم يجب أن يكون نصا ';
            $arr['code.required'] = ' كود الخصم مطلوب ';
            $arr['code.unique'] = ' كود الخصم يحتوي علي قيم مستخدمة من قبل ';

            if(! empty(request()->apply_ids)) {
                foreach(request()->apply_ids as $key => $key2) {

                 
                    $arr['apply_ids.'.$key.'.exists'] = ' العنصر المختار '  .($key + 1). ' غير موجود ';
