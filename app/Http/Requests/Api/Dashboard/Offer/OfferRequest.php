...
namespace App\Http\Requests\Api\Dashboard\Offer;
...
use App\Rules\Api\Dashboard\Offer\ApplyOnRule;
...
class OfferRequest extends ApiMasterRequest
{
                    ...
                    foreach ($productDetails as $productDetail) {
                        if ($productDetail and $productDetail->inActiveFlashSale()) {
                            throw new HttpResponseException(response()->json([
                                'status' => 'fail',
                                'data' => null,
                                'message' => $lang == 'en' ? 'cant add this product because it in flash sale' : 'لا يمكن إضافة هذا القسم لانه يحتوي علي منتج  ضمن الخصومات المفاجأة ',
                    ...
                    foreach ($this->buy_apply_ids as $buy_apply_id) {
                        ...
                        if ($product_details and $product_details->inActiveFlashSale()) {
                            throw new HttpResponseException(response()->json([
                                'status' => 'fail',
                                'data' => null,
                                'message' => $lang == 'en' ? 'cant add this product because it in flash sale' : 'لا يمكن إضافة هذا المنتج لأنه ضمن الخصومات المفاجأة ',
                            ], 422));
                        }
                    }
                }
                ...
                foreach ($this->apply_ids as $apply_id) {
                    ...
                    if($offerItemDetail != null){
                        throw new HttpResponseException(response()->json([
                            'status' => 'fail',
                            'data' => null,
                            'message' => $lang == 'en' ? 'cant add this product because it in offer' : 'لا يمكن إضافة هذا المنتج لانه يحتوي علي منتج  ضمن العروض ',
    ...
    public function messages()
    {
        ...
        if($lang == 'en') {
            ...
            $arr['start_at.required'] = ' start at is required';
            $arr['start_at.date'] = 'start at is must be date';
            $arr['start_at.after'] = ' start at must be after now';

            $arr['end_at.required'] = ' end at is required';
            $arr['end_at.date'] = 'end at is must be date';
            $arr['end_at.after'] = ' end at must be after start date';

            $arr['payment_type.required_if'] = ' payment type required if is apply on or special payment';
            $arr['payment_type.in'] = 'payment type must be wallet or card or cash';
            ...
            $arr['apply_ids.required_if'] = 'required if apply on is special products , special categories , special payment';

        } else {

            $arr['start_at.required'] = ' تاريخ البداية مطلوب ';
            $arr['start_at.date'] = 'تاريخ البداية يجب ان يكون تاريخ';
            $arr['start_at.after'] = ' تاريخ البداية يجب ان يكون بعد الأن ';

            $arr['end_at.required'] = ' تاريخ النهاية مطلوب ';
            $arr['end_at.date'] = 'تاريخ النهاية يجب ان يكون تاريخ';
            $arr['end_at.after'] = ' تاريخ النهاية يجب ان يكون بعد تاريخ البداية ';

            $arr['payment_type.required_if'] = ' نوع الدفع مطلوب لو الدفع عبر التقديم أو دفع خاص';
            $arr['payment_type.in'] = 'نوع الدفع يجب أن يكون عن طريق المحفظة أو كاش أو الفيزا';
            ...
            $arr['apply_ids.required_if'] = 'منتجات العرض مطلوبة في حال اذا كان نوع العرض منتجات مخصصة , أقسام مخصصة, دفع مخصص ';
        }
