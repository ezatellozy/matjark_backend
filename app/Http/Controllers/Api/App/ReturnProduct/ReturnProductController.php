...
namespace App\Http\Controllers\Api\App\ReturnProduct;

use App\Http\Controllers\Controller;
use App\Http\Requests\Api\App\ReturnProduct\ReturnProductRequest;
use App\Models\Order;
use App\Models\OrderProduct;
use App\Models\ReturnOrder;
use App\Models\User;
use App\Http\Resources\Api\App\Order\{OrderResource};
...
class ReturnProductController extends Controller
{
  public function  store(ReturnProductRequest $request)
  {
    ...
    if ($order->returnOrder && $order->returnOrder->status == 'waiting') {
      return response()->json(['status' => 'fail', 'message' => trans('app.messages.wait_for_admin_accept_reject_your_request'), 'data' => null], 422);
    }
    ...
    if ($order->updated_at->diff(now())->days >   $product_return_grace_period) {
      return response()->json(['status' => 'fail', 'message' => trans('app.messages.return_period_has_expired'), 'data' => null], 422);
    }
    if ($order->orderCoupon != null || $order->flashSaleOrders->count() > 0 || $order->offerOrders->count() > 0) {
      return response()->json(['status' => 'fail', 'message' => trans('app.messages.offered_product_cannot_be_returned'), 'data' => null], 422);
    }
    ...
    foreach ($orderProducts as $orderProduct) {
      ...
      if ($product->quantity <  $orderProduct['quantity']) {
        return response()->json(['status' => 'fail', 'message' => trans('app.messages.confirm_the_quantity_to_be_returned'), 'data' => null], 422);
      }
    }
    \DB::beginTransaction();
    ...
    try {
      ...
      //     $admin->notify(new ReturnOrderNotification($order, ['database']));
      //   }
      // }
