...
namespace App\Http\Controllers\Api\Dashboard\Wallet;

use App\Http\Controllers\Controller;
use App\Http\Requests\Api\Dashboard\Wallet\WalletChargeRequest;
use App\Http\Resources\Api\Dashboard\Wallet\WalletTransactionResource;
use App\Models\User;
...
class WalletController extends Controller
{
    ...
    public function charge(WalletChargeRequest $request, $id)
    {
        ...
        if ($request->type == 'withdrawal' && $request->amount > $user->wallet->balance) {
            return response()->json(['data' => null, 'status' => 'fail', 'message' => trans('dashboard.message.amount_bigger_than_wallet')], 422);
        }
    ...
    public function balance($request, $user, $type)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['data' => null, 'status' => 'success', 'message' => trans('dashboard.wallet.' . '.' . $type, ['amount' => $request->amount])]);
        } catch (Exception $e) {
            DB::rollBack();
